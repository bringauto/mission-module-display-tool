...
This tool is used to display the positions of cars on a map using the [fleet protocol HTTP API](https://github.com/bringauto/fleet-protocol-http-api).
...
It will start a web server on `http://localhost:5000/` and display a map with cars.
...
If the `--config` option is not set, the default configuration file `config/config-docker.json` is used.

### How to use with [bringauto/etna](https://github.com/bringauto/etna)
